"""The tests for the tpi_thermostat."""
...
from custom_components.tpi_thermostat.const import (
    DOMAIN as tpi_thermostat_DOMAIN,
)
from tests.climate.common import (
    async_set_temperature,
    async_set_preset_mode,
    async_set_hvac_mode,
)
from tests.common import get_fixture_path
from tests.switch.common import MockSwitch
...
async def test_valid_conf(hass: HomeAssistant) -> None:
    """Test set up tpi_thermostat with valid config values."""
...
async def test_set_target_temp(hass: HomeAssistant) -> None:
    ...
    await async_set_temperature(hass, 30)
    state = hass.states.get(ENTITY)
    assert state.attributes.get("temperature") == 30.0
    with pytest.raises(vol.Invalid):
        await async_set_temperature(hass, None)
    state = hass.states.get(ENTITY)
    assert state.attributes.get("temperature") == 30.0
...
async def test_set_away_mode(hass: HomeAssistant, preset, temp) -> None:
    ...
    await async_set_temperature(hass, 23)
    await async_set_preset_mode(hass, preset)
    state = hass.states.get(ENTITY)
...
async def test_set_away_mode_and_restore_prev_temp(
    ...
    state = hass.states.get(ENTITY)
    assert state.attributes.get("temperature") == temp
    await async_set_preset_mode(hass, PRESET_NONE)
    state = hass.states.get(ENTITY)
...
async def test_set_preset_mode_invalid(hass: HomeAssistant) -> None:
    ...
    await async_set_temperature(hass, 23)
    await async_set_preset_mode(hass, "away")
    state = hass.states.get(ENTITY)
    assert state.attributes.get("preset_mode") == "away"
    await async_set_preset_mode(hass, "none")
    state = hass.states.get(ENTITY)
    assert state.attributes.get("preset_mode") == "none"
    with pytest.raises(ServiceValidationError):
        await async_set_preset_mode(hass, "Sleep")
    state = hass.states.get(ENTITY)
    assert state.attributes.get("preset_mode") == "none"
...
async def test_running_when_hvac_mode_is_off(hass: HomeAssistant) -> None:
    ...
    calls = _setup_switch(hass, True)
    await async_set_temperature(hass, 30)
    await async_set_hvac_mode(hass, HVACMode.OFF)
    assert len(calls) == 1
    call = calls[0]
    assert call.domain == HOMEASSISTANT_DOMAIN
    assert call.service == SERVICE_TURN_OFF
...
async def test_hvac_mode_heat(hass: HomeAssistant) -> None:
    ...
    await hass.async_block_till_done()
    calls = _setup_switch(hass, False)
    await async_set_hvac_mode(hass, HVACMode.HEAT)
    assert len(calls) == 1
    call = calls[0]
    assert call.domain == HOMEASSISTANT_DOMAIN
    assert call.service == SERVICE_TURN_ON
...
def _setup_switch(hass: HomeAssistant, is_on: bool) -> list[ServiceCall]:
    ...
    @callback
    def turn_on(call: ServiceCall):
        hass.states.async_set(ENT_SWITCH, STATE_ON)
        calls.append(call)

    @callback
    def turn_off(call: ServiceCall):
        hass.states.async_set(ENT_SWITCH, STATE_OFF)
        calls.append(call)

    hass.services.async_register(ha.DOMAIN, SERVICE_TURN_ON, turn_on)
    hass.services.async_register(ha.DOMAIN, SERVICE_TURN_OFF, turn_off)
...
async def test_turn_away_mode_on_cooling(hass: HomeAssistant) -> None:
    ...
    await async_set_preset_mode(hass, PRESET_AWAY)
    state = hass.states.get(ENTITY)
...
async def test_hvac_mode_cool(hass: HomeAssistant) -> None:
    ...
    await hass.async_block_till_done()
    calls = _setup_switch(hass, False)
    await async_set_hvac_mode(hass, HVACMode.COOL)
    assert len(calls) == 1
    call = calls[0]
    assert call.domain == HOMEASSISTANT_DOMAIN
    assert call.service == SERVICE_TURN_ON
...
#
#     # When
#     await async_set_temperature(hass, target_temperature)
...
#     # Then
#     assert len(calls) == 0
#     await async_set_hvac_mode(hass, changed_hvac_mode)
#     assert len(calls) == 1
#     call = calls[0]
...
async def test_precision(hass: HomeAssistant) -> None:
    ...
    await async_set_temperature(hass, 55.27)
    state = hass.states.get(ENTITY)
...
async def test_state_1(hass: HomeAssistant) -> None:
    """Test that correct selected TPI state."""
    calls = _setup_switch(hass, False)

    _setup_sensor(hass, 22.4)
    await hass.async_block_till_done()
    assert len(calls) == 1

    state = hass.states.get(ENTITY)
    assert state.attributes.get("thermostat_state") == "state_1"


@pytest.mark.usefixtures("setup_comp_10")
async def test_state_1_to_tpi(hass: HomeAssistant) -> None:
    """Test that correct selected TPI state."""

    test_time = datetime.datetime.now(dt_util.UTC)

    with patch("custom_components.tpi_thermostat.climate.datetime") as mock_datetime:
        calls = _setup_switch(hass, False)
        ...
        await hass.async_block_till_done()
        assert len(calls) == 1
        assert calls[0].domain == HOMEASSISTANT_DOMAIN
        assert calls[0].service == SERVICE_TURN_ON
        assert calls[0].data["entity_id"] == ENT_SWITCH
        assert hass.states.get(ENT_SWITCH).state == STATE_ON

        mock_datetime.now.return_value = test_time + datetime.timedelta(minutes=5)
        async_fire_time_changed(hass, test_time + datetime.timedelta(minutes=5))
        _setup_sensor(hass, 22.9)
        await hass.async_block_till_done()

        mock_datetime.now.return_value = test_time + datetime.timedelta(minutes=10)
        async_fire_time_changed(hass, test_time + datetime.timedelta(minutes=15))
        _setup_sensor(hass, 23.6)
        await hass.async_block_till_done()
        assert len(calls) == 2
        assert calls[1].domain == HOMEASSISTANT_DOMAIN
        assert calls[1].service == SERVICE_TURN_OFF
        assert calls[1].data["entity_id"] == ENT_SWITCH

        state = hass.states.get(ENTITY)
        assert state.attributes.get("slope")

        mock_datetime.now.return_value = test_time + datetime.timedelta(minutes=20)
        async_fire_time_changed(hass, test_time + datetime.timedelta(minutes=20))
        _setup_sensor(hass, 22.9)
        await hass.async_block_till_done()

        mock_datetime.now.return_value = test_time + datetime.timedelta(minutes=25)
        async_fire_time_changed(hass, test_time + datetime.timedelta(minutes=25))
        _setup_sensor(hass, 22.4)
        await hass.async_block_till_done()
        assert len(calls) == 3
        assert calls[2].domain == HOMEASSISTANT_DOMAIN
        assert calls[2].service == SERVICE_TURN_ON
        assert calls[2].data["entity_id"] == ENT_SWITCH

        state = hass.states.get(ENTITY)
        assert state.attributes.get("thermostat_state") == "state_tpi"


@pytest.mark.usefixtures("setup_comp_10")
async def test_state_2(hass: HomeAssistant) -> None:
    """Test that correct selected TPI state."""
    calls = _setup_switch(hass, False)
    ...
    state = hass.states.get(ENTITY)
    assert state.attributes.get("thermostat_state") == "state_2"


@pytest.mark.usefixtures("setup_comp_10")
async def test_state_3(hass: HomeAssistant) -> None:
    """Test that correct selected TPI state."""
    calls = _setup_switch(hass, False)
    ...
    state = hass.states.get(ENTITY)
    assert state.attributes.get("thermostat_state") == "state_3"


@pytest.mark.usefixtures("setup_comp_10")
async def test_state_3_tpi(
    hass: HomeAssistant, entity_registry: er.EntityRegistry
) -> None:
    """Test that correct selected TPI state."""
    calls = _setup_switch(hass, False)
...
async def test_reload(hass: HomeAssistant) -> None:
    ...
    yaml_path = get_fixture_path("configuration.yaml", "tpi_thermostat")
    with patch.object(hass_config, "YAML_CONFIG_FILE", yaml_path):
        await hass.services.async_call(
            tpi_thermostat_DOMAIN,
            SERVICE_RELOAD,
            {},
            blocking=True,
        )
